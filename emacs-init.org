...
** Paradox for package install UI
...
TODO - This could be moved somewhere else and not have its own section
...
This could also be moved into another section since this is a single configuration item.
...
These are useful for working within projects, so perhaps this section would be better served in there.
...
;; Completion & Search
(require 'init-ivy)
(require 'init-helm)     ; Note: ivy and helm overlap - consider choosing one
(require 'init-smex)
